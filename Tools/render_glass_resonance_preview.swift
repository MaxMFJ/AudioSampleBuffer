...
let vinyl=try pipeline("vinylDiscVertex","vinylDiscFragment",.rgba16Float,true)
...
func render(_ name:String,_ state:GlassAudioState,_ side:Int=900,_ aspect:Float=1)->[UInt8] {
 ...
 u[364]=state.phase;u[365]=state.strain;u[366]=state.climax;u[367]=state.energy;u[368]=0.32;u[369]=1.05
 u[380]=state.rays.envelope;u[381]=state.rays.age
 ...
 e.setDepthStencilState(depthState);e.setFrontFacing(.counterClockwise);e.setCullMode(.back);e.setRenderPipelineState(vinyl);e.drawPrimitives(type:.triangle,vertexStart:0,vertexCount:96*6*4)
 e.setCullMode(.front);e.setRenderPipelineState(rearPipeline);e.drawIndexedPrimitives(type:.triangle,indexCount:indices.count,indexType:.uint16,indexBuffer:indexBuffer,indexBufferOffset:0,instanceCount:2);e.endEncoding()
 ...
 front.setDepthStencilState(depthState);front.setFrontFacing(.counterClockwise);front.setCullMode(.back);front.setRenderPipelineState(vinyl);front.drawPrimitives(type:.triangle,vertexStart:0,vertexCount:96*6*4)
 front.setRenderPipelineState(glass);front.setFragmentTexture(rear,index:0);front.drawIndexedPrimitives(type:.triangle,indexCount:indices.count,indexType:.uint16,indexBuffer:indexBuffer,indexBufferOffset:0,instanceCount:2);front.endEncoding()
...
precondition(silent.climax<0.02 && climax.climax>0.45,"Climax envelope does not separate quiet from chorus")
var live=state(0..<80,0.08)
precondition(live.climax<0.08,"Verse-level spectrum must stay off")
var hot=state(0..<80,0.15)
precondition(hot.climax>0.15,"High-energy song stays dark")
var lift=state(0..<80,0.30)
precondition(lift.climax>0.20,"Loud mix does not unlock climax")
var treble=state(53..<80,0.55)
precondition(treble.climax>0.12,"Treble-heavy chorus does not unlock climax")
let climaxImage=render("climax",climax)
precondition(zip(musicImage,climaxImage).filter{$0 != $1}.count>1000,"Climax failed to change the Metal render")
...
var flipMore=music;flipMore.phase=2.4;_ = render("rotation-late",flipMore)
...
precondition(stale.impact==0 && stale.low==0 && stale.strain==0 && stale.climax==0)
...
// Overlapping beats cannot restack until the whoosh finishes and a quiet gap elapses.
var lock=GlassAudioState();lock.quietAge=0.10
let burst=[Float](repeating:0.7,count:80)
glassAudioStep(&lock,burst,0.9,0.8,0,1,1.15,1/30)
let firstAmp=lock.kickAmp
precondition(firstAmp>0.10,"Kick did not arm after a quiet gap")
var peak:Float=0
for _ in 0..<8 {glassAudioStep(&lock,burst,0.9,0.8,0,1,1.15,1/30);peak=max(peak,lock.kick)}
precondition(abs(lock.kickAmp-firstAmp)<0.001 && lock.kickClock>0.2 && lock.kickClock<0.42,"Kick restacked before the whoosh finished")
while lock.kickAmp>0.001 {
 glassAudioStep(&lock,burst,0.9,0.8,0,1,1.15,1/30)
 peak=max(peak,lock.kick)
}
precondition(peak>firstAmp*0.8 && lock.kick==0)
glassAudioStep(&lock,burst,0.9,0.8,0,1,1.15,1/30)
precondition(lock.kickAmp==0,"Kick armed without a quiet gap")
for _ in 0..<4 {glassAudioStep(&lock,zeros,0,0,0,0,1.15,1/30)}
glassAudioStep(&lock,burst,0.9,0.8,0,1,1.15,1/30)
precondition(lock.kickAmp>0.10,"Kick did not rearm after a quiet gap")
print("PASS: independent bands, impact, silent stale features, sustain rejection, silence release, 24/60 fps, finite values, one-shot kick lock.")
