...
fragment float4 fractalPatternFragment(RasterizerData in [[stage_in]],
    ...
    // 🎯 低音冲击检测 - 强化触发和衰减
    float bassImpact = smoothstep(0.2, 0.7, bassIntensity); // 降低阈值，更容易触发
    float impactDecay = exp(-fmod(time * 10.0, 6.28) * 1.8); // 提高频率，更快速的冲击
    float shockWave = bassImpact * impactDecay * 4.0; // 增加2倍强度
    
    // 🫧 呼吸式脉动 - 降低浮动（更稳定）
    float breathPhase = sin(time * 0.8) * 0.5 + 0.5;
    float breathScale = 1.0 + breathPhase * 0.06 + bassIntensity * 0.35 + shockWave * 0.7; // 降低呼吸浮动从0.15到0.06
    ...
    for (float y = -1.0; y <= 1.0; y += 1.0) {
        for (float x = -1.0; x <= 1.0; x += 1.0) {
            ...
            // 💥 低音冲击 - 剧烈快速抖动（增强版）
            float impactJitter = shockWave * 2.8; // 从0.8提升到1.8，抖动幅度翻倍
            float impactSpeed = 5; // 从15提升到25，更快速的抖动
            
            // 重击时的随机偏移增强
            float2 impactRandomOffset = (hash22(cellID + time * 10.0) - 0.5) * impactJitter * 0.5;
            ...
            // 柔和的点光晕 - 低音时点明显变大
            float glowRadius = 10.0 - bassIntensity * 3.5 - shockWave * 2.0; // 低音和冲击时点更大
            float pointGlow = exp(-dist * glowRadius) * 0.7; // 降低基础亮度避免过曝
            
            // 🔊 低音增强亮度 - 极致增强版
            float bassBrightness = 1.0 + bassIntensity * 2.5 + shockWave * 15.0; // 超强亮度响应
            ...
            // 🎨 优化粒子颜色 - 蓝紫色调更柔和
            float hue = hash21(cellID) * 0.2 + time * 0.08 + averageAudio * 0.15;
            hue += midIntensity * 0.1; // 中音改变色相
            // 蓝紫色基调：0.6-0.7范围（蓝色到紫色）
            hue = fract(hue + 0.62); // 从0.55调整到0.62，更偏蓝紫色
            float saturation = 0.75 + midIntensity * 0.2; // 提高基础饱和度，更鲜艳
            float brightness = 1.0 + bassIntensity * 0.4 + shockWave * 0.6; // 低音和冲击增强亮度
            float3 pointColor = hsv2rgb(float3(hue, saturation, brightness));
    ...
    volumeFog = pow(volumeFog, 1.4) * 0.4; // 从1.5改为1.4，强度从0.3提升到0.4
    ...
    sphereMask = pow(sphereMask, 0.8); // 让渐变更柔和明显
    
    // 🌊 呼吸波纹 - 从中心扩散（增强版）
    float ripple = sin(centerDist * 20.0 - time * 4.0) * 0.5 + 0.5;
    ripple = pow(ripple, 2.5) * midIntensity * 0.9; // 从pow(3.0)改为2.5，强度从0.5提升到0.9
    ...
    midRing = pow(midRing, 3.5) * midIntensity * 1.0; // 从pow(4.0)改为3.5，强度从0.6提升到1.0
    ...
    finalColor += ripple * float3(0.9, 0.95, 1.0) * 1.4; // 蓝白色波纹，强度提升40%
    
    // 🎸 中音能量环（暖色调增强）
    finalColor += midRing * float3(0.95, 0.7, 1.0) * 1.3; // 粉紫色，强度提升30%
    ...
    edgeGlow *= (0.6 + averageAudio * 0.6); // 从0.5提升到0.6，更明显
    finalColor += edgeGlow * float3(0.7, 0.85, 1.0) * 1.1; // 从0.8提升到1.1
    
    // 💥 低音冲击闪光（超强版）
    float impactFlash = shockWave * exp(-centerDist * 2.0); // 进一步扩大范围
    finalColor += impactFlash * float3(0.9, 0.95, 1.0) * 2.5; // 冷白色，强度从1.2提升到2.5
    
    // 🎭 半透明度控制 - 包含所有特效
    finalAlpha = saturate(finalAlpha * sphereMask * 0.7 + volumeFog + ripple + midRing + trebleBeams * 0.5);
